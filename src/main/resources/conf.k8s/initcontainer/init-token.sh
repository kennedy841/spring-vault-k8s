...
if [[ -z $VAULT_TOKEN ]]; then
    ...
    echo "found login token: ${VAULT_LOGIN_TOKEN}"
    ...
    validateVaultResponse 'secret id' "${VAULT_SECRET_ID}"
     ...
     echo "found vault secret: ${VAULT_SECRET_ID}"
    ...
    validateVaultResponse 'approle id' "${VAULT_TOKEN}"
     ...
     echo "found vault token: ${VAULT_TOKEN}"
fi
...
do
  ...
  if ! grep "export ${output_key}=" ${VARIABLES_FILE} > /dev/null; then
    ...
    echo "Getting secret not in variables ${output_key}"

    vault_response=$(retrieveSecret ${VAULT_TOKEN} ${vault_key})

    echo "retrieveSecret with token ${VAULT_TOKEN} in ${vault_key}"

    matching_secret_keys=$(printenv | grep -e "=${vault_key}" | awk -F "=" '{print $1}')
    echo "matching_secret_keys ${matching_secret_keys}"
